...
export default function InvoiceForm({ onSubmit }: InvoiceFormProps) {
  ...
  useEffect(() => {
    ...
    if (savedData) {
      const { invoiceDate, customerName, customerPhone, goods, lastId, invoiceCode } = JSON.parse(savedData);
  ...
  }, [invoiceDate, customerName, customerPhone, goods, invoiceCode]);
    ...
    <form onSubmit={handleSubmit} className="bg-white p-8 rounded-xl shadow-lg max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-200 -mx-8 md:mx-0">
          <div className="min-w-[900px]"> {/* Minimum width container */}
            <table className="w-full divide-y divide-gray-200">
              <thead>
                <tr className="bg-gray-50">
                  <th className="w-[80px] px-4 py-3 text-left text-sm font-semibold text-gray-700"></th>
                  <th className="w-[60px] px-4 py-3 text-left text-sm font-semibold text-gray-700">STT</th>
                  <th className="w-[300px] px-4 py-3 text-left text-sm font-semibold text-gray-700">Tên</th>
                  <th className="w-[120px] px-4 py-3 text-left text-sm font-semibold text-gray-700">Số thùng</th>
                  <th className="w-[120px] px-4 py-3 text-left text-sm font-semibold text-gray-700">ĐV/thùng</th>
                  <th className="w-[120px] px-4 py-3 text-left text-sm font-semibold text-gray-700">Tổng đơn vị</th>
                  <th className="w-[100px] px-4 py-3 text-center text-sm font-semibold text-gray-700">Xóa</th>
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {goods.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="w-[60px] px-4 py-4 text-sm text-gray-700 text-center">{index + 1}</td>
                    ...
                    <td className="w-[120px] px-4 py-4">
                      <input
                        ...
                        onChange={(e) => updateGoodsItem(item.id, 'itemsPerPackage', parseInt(e.target.value) || 0)}
                        onKeyDown={(e) => {
                          if (e.key === '-' || e.key === 'e') {
                            e.preventDefault();
                          }
                    ...
                    <td className="w-[120px] px-4 py-4">
                      <input
                        ...
                        onChange={(e) => updateGoodsItem(item.id, 'totalUnits', parseInt(e.target.value) || 0)}
                        onKeyDown={(e) => {
                          if (e.key === '-' || e.key === 'e') {
                            e.preventDefault();
                          }
      ...
      <div className="bg-gray-50 p-6 rounded-lg border border-gray-200">
        ...
        <div className="space-y-2">
          ...
          <p className="text-gray-700">
            Tổng đơn vị: <span className="font-bold text-gray-900">{calculateTotals().totalUnits}</span>
          </p>
        </div>
      </div>
    </form>
